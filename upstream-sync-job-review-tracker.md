...
| P0.2c | P0 | **NEW (found during #4043 gate, 2026-07-14).** SalesforceSDKCore has a large pre-existing crash cluster (~52 cases / ~20 classes) in auth/credentials/user-account/push/network tests. Root cause = same `OAuthCredentials(...)!` convenience-init-returns-nil bug fixed for SmartStore in `f11e4754f`; SDKCore test helpers still use it. NOT baselined → SDKCore gate provisional. | ☐ open | Fix = apply `f11e4754f`-style `.credentials(...)` factory across SDKCore test helpers. #4043 accepted via scoped evidence (build ✓, IDP tests pass, no failure touches changed code). See `.claude/test-baseline.md` P0.2c block. |
