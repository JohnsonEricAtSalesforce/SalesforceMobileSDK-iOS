...
| P0.2e | P0 | **NEW (found during P0.2d fix, 2026-07-14).** Removing the P0.2d crashes stopped the host-restart cascade and unmasked **57 pre-existing failing tests** (same phenomenon as P0.2b). Full suite: 518 pass / 57 fail / 0 in-scope crashes. Concentrated in PushNotificationManagerTests (13), SFSDKEncryptedPushNotificationTests (12), SFUserAccountManagerTests (6), Biometric (6, incl. 2 NEW `.optIn!` force-unwrap crashes), Persister (4, incl. 2 NEW index-out-of-range). NOT baselined → SDKCore gate provisional. | ☐ open | Needs triage: which are real migration regressions vs. test-isolation artifacts. Two new non-P0.2d crashes to root-cause (`BiometricAuthenticationManagerTests.swift:85` nil-unwrap; `SFUserAccountManagerPersisterTests.testMultipleAccounts` accounts array). See `.claude/test-baseline.md` P0.2e block. |
