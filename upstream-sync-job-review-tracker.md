...
| P1.5 | P1 | Analysis not durable before work begins: scalar marker can't track out-of-order progress, and the expensive Cat-B intent/Swift-mapping analysis evaporates on compaction. Raised by operator (high-volume backlog, costly per-unit commit→contribution mapping). | ☑ done | Applied 2026-07-14: new `.claude/upstream-sync-backlog.md` ledger — per-unit status vocab + persisted Cat-B detail blocks; marker reframed as linear done-floor + composition rule (advance only when units ≤X are ported/skipped); poller step 8 merge-not-clobber (operator chose); Cat-B workflow writes analysis to ledger; activation seeds ledger + "analyze whole backlog first" pre-work. Changes-from-v1 row #8. |
